...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for pos in self.legalPositions:
            ...
            # Get the weights out of the emission table which is P(e | x)
            trueDistance = util.manhattanDistance(pos, pacmanPosition)
            probE_X = emissionModel[trueDistance]
            allPossible[pos] = probE_X * self.beliefs[pos]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # initiate blank distribution
        main_position_dist = util.Counter()
        # for all old legal positions...
        for old_position in self.legalPositions:
            # get ghost's new position distribution given old position
            new_position_dist = self.getPositionDistribution(self.setGhostPosition(gameState, old_position))
            # add to probability of new positions given new probability * old belief
            for newPos, prob in new_position_dist.items():
                main_position_dist[newPos] += prob * self.beliefs[old_position]

        self.beliefs = main_position_dist
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            for particle in range(1, particlesPerPos):
    ...
    def observe(self, observation, gameState):
        ...
        # Elapse particles
        # Sample from the belief distribution for each particle
        for i in range(0, len(self.particleList)):
            self.particleList[i] = util.sample(oldBeliefs)

        # Observe the particles by weighting them
        # w(x) = P(e|x)
        # B(x) = w(x)P(e|x)
        for pos in self.legalPositions:
            trueDistance = util.manhattanDistance(pos, pacmanPosition)
            probE_X = emissionModel[trueDistance]
            newBeliefs[pos] = probE_X * oldBeliefs[pos]
        newBeliefs.normalize()

        if newBeliefs.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            # Resample the particles and form a newParticle list
            for i in range(0, len(self.particleList)):
                self.particleList[i] = util.sample(newBeliefs)
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        newParticleList = util.Counter()

        for particle in self.particleList:
            newParticleList[particle] += 1

        newParticleList.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # ghosts may occupy at self.legalPositions
        particle_list = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        # initialize particles to be consistent with a uniform prior.
        self.particles = []
        for count in range(0, self.numParticles):
            # choice = random.choice(particle_list)
            self.particles.append(particle_list[count % len(particle_list)])
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            likelihood = 1.0
            for i in range(self.numGhosts):
                # edge case: when a ghost is captured by Pacman
                if noisyDistances[i] is None:
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    true_distance = util.manhattanDistance(pacmanPosition, particle[i])
                    conditioned_evidence = emissionModels[i][true_distance]
                    likelihood = likelihood * conditioned_evidence
            new_belief[particle] += likelihood


        # edge case: when all particles receive 0 weight.
        if new_belief.totalCount() == 0:
            self.initializeParticles()
            for particle in self.particles:
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None:
                        new_particle = self.getParticleWithGhostInJail(particle, i)
                        self.particles[particle] = new_particle
        # re-sample: select prior samples (tuples) in the proportion to their likelihood
        else:
            new_belief.normalize()
            new_particles = []
            for count in range(0, self.numParticles):
                new_particles.append(util.sample(new_belief))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            main_position_dist = util.Counter()
            # for all old legal positions...
            for ghost_number in range(0, self.numGhosts):
                # get ghost's new position distribution given old position
                new_position_dist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, newParticle), ghost_number, self.ghostAgents[ghost_number])
                # sample the new position distribution for each ghost
                newParticle[ghost_number] = util.sample(new_position_dist)
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_distribution = util.Counter()
        # accumulate particles for each position
        for position in self.particles:
            new_distribution[position] += 1
        # normalize
        new_distribution.normalize()
        # for position in new_distribution:
        #     new_distribution[position] = new_distribution[position] / float(self.numParticles)
